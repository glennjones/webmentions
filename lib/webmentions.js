...
module.exports = {
	...
	discoverEndpoint: function ( options, callback ){
		...
		console.log('webmentions microformat-node version: ', microformats.version);

		if(!microformats.version){
			console.log('webmentions microformat-node version: NO VERSION');
		}
			...
			if(!err){
				...
				parseEndpoint(options.target, response, body, dom, node, function( err, webmention ){
					if( !err ){
					    ...
					    if(options.proxy){
					    	out.target.proxy = options.proxy;
					    }
					    count++
					    release();
					}else{
...
function parseEndpoint(url, response, body, dom, node, callback){
	...
	console.log('webmentions body', body)
	console.log('webmentions dom', dom)
	console.log('webmentions dom', node)

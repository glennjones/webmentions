...
module.exports = {
	...
	getMention: function( url, callback){
		callback( null, {} );
	},
	...
	validateMention: function ( options, callback ){
		...
		this.getMentionData( options, function( err, data ){
			if(!err && data){
		...
		//this.getMentionData( options, callback );
	},
...
function getURLContent( options, callback ){
	if(options.url){
		...
		if(options.url === undefined 
			...
			callback( utils.buildError( 400, 'request error', 'The url seems not to be in the correct formatted'), null  );
		}
		...
		request(requestOptions, function(requestErrors, response, body) {
			if (!requestErrors && response.statusCode === 200) {
				console.log('worked:', options.url)
				...
				if(response && response.statusCode && response.statusCode > 399){
					...
					callback( utils.buildError( 400, 'request error', requestErrors + ' - ' + options.url), null  );
				}
			}
		}); 
